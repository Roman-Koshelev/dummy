...
static const char *a615a_file_ext[16] = {"LCI", "LCL", "LCS", "LNA", "LND", "LNL", "LNO", "LNR",
...
static proto_tree *dissect_a615a_header(tvbuff_t *tvb, packet_info *pinfo, int *offsetPtr,
                                        proto_tree *tftp_tree, char *file, char *ext);
...
static void dissect_a615a_LUI_Common(tvbuff_t *tvb, packet_info *pinfo, int offset,
                                     proto_tree *root);
...
static void dissect_a615a_a665_msg(tvbuff_t *tb, packet_info *pinfo, int offset,
                                   const char *a665Str, proto_tree *tftp_tree);
                                        ...
                                        proto_tree *tftp_tree, char *file, char *ext)
{
    ...
    proto_tree *root;
    root = proto_tree_add_subtree_format(tftp_tree, tvb, offset, -1, ett_a615a_protocol_root, NULL,
                                         "%s (%s)", file, ext);
    /* file length */
...
{
    proto_tree *root =
        dissect_a615a_header(tvb, pinfo, &offset, tftp_tree, "Load Configuration List", "LCL");
    gint end;
    ...
    for (unsigned i = 0; i < numTgtHardware; i++) {
        ...
        if (numLength > 0) {
            ...
            char *serialNum =
                tvb_get_stringz_enc(wmem_packet_scope(), tvb, offset, &end, ENC_ASCII);
        ...
        for (unsigned i = 0; i < numPartNumbers; i++) {
            ...
            part_root = proto_tree_add_subtree_format(root, tvb, partNumLengthOffset, -1,
                                                      ett_a615a_protocol_root, NULL, "Part %d - %s",
                                                      i + 1, pnum);
            proto_tree_add_uint(part_root, hf_a615a_part_number_length, tvb, partNumLengthOffset, 1,
                                partNumberLength);
            proto_tree_add_string(part_root, hf_a615a_part_number, tvb, partNumOffset,
                                  partNumberLength, pnum);
            ...
            if (ammendLen > 0) {
                ...
                char *ammend =
                    tvb_get_stringz_enc(wmem_packet_scope(), tvb, offset, &end, ENC_ASCII);
                proto_tree_add_string(part_root, hf_a615a_ammendment, tvb, offset, ammendLen,
                                      ammend);
                offset += ammendLen;
            }
...
static void dissect_a615a_LUS(tvbuff_t *tvb, packet_info *pinfo, int offset, proto_tree *tftp_tree)
{
    gint end;
    proto_tree *root =
        dissect_a615a_header(tvb, pinfo, &offset, tftp_tree, "Load Upload Status", "LUS");
    ...
    col_append_fstr(pinfo->cinfo, COL_INFO, ", Status: %s",
                    val_to_str(opCode, a615a_op_status_codes, "Unknown (0x%04x)"));
    ...
    for (unsigned i = 0; i < count; i++) {
        ...
        proto_tree *part_root;
        part_root =
            proto_tree_add_subtree_format(root, tvb, fnameLengthOffset, -1, ett_a615a_protocol_root,
                                          NULL, "Header %d - %s", i + 1, fname);

        proto_tree_add_uint(part_root, hf_a615a_file_name_length, tvb, fnameLengthOffset, 1,
                            fnameLength);
        proto_tree_add_string(part_root, hf_a615a_file_name, tvb, fnameOffset, fnameLength, fname);

        /* part number length */
        guint8 partNumberLength = tvb_get_guint8(tvb, offset);
        proto_tree_add_uint(part_root, hf_a615a_part_number_length, tvb, offset, 1,
                            partNumberLength);
        offset += 1;
        ...
        if (partNumberLength > 0) {
            ...
            gint end = partNumberLength;
            char *pnum = tvb_get_stringz_enc(wmem_packet_scope(), tvb, offset, &end, ENC_ASCII);
            proto_tree_add_string(part_root, hf_a615a_part_number, tvb, offset, partNumberLength,
                                  pnum);
            offset += partNumberLength;
        }
        ...
        proto_tree_add_uint(part_root, hf_a615a_status_description_length, tvb, offset, 1,
                            statLength);
        offset += 1;
        ...
        if (statLength > 0) {
            ...
            char *statDesc = tvb_get_stringz_enc(wmem_packet_scope(), tvb, offset, &end, ENC_ASCII);
            proto_tree_add_string(part_root, hf_a615a_status_description, tvb, offset,
                                  tvb_captured_length_remaining(tvb, offset), statDesc);
            offset += statLength;
        }
    }
}
...
{
    proto_tree *root =
        dissect_a615a_header(tvb, pinfo, &offset, tftp_tree, "Load Control Status", "LCS");
    /* counter */
    guint16 counter = tvb_get_ntohs(tvb, offset);
    proto_tree_add_uint(root, hf_a615a_counter, tvb, offset, 2, counter);
    offset += 2;
    ...
    if (statLength > 0) {
        ...
        gint end = statLength;
        char *statDesc = tvb_get_stringz_enc(wmem_packet_scope(), tvb, offset, &end, ENC_ASCII);
        proto_tree_add_string(root, hf_a615a_status_description, tvb, offset,
                              tvb_captured_length_remaining(tvb, offset), statDesc);
        offset += end;
    }
}
...
{
    proto_tree *root =
        dissect_a615a_header(tvb, pinfo, &offset, tftp_tree, "Load Upload Initialization", "LUI");
...
static void dissect_a615a_LCI(tvbuff_t *tvb, packet_info *pinfo, int offset, proto_tree *tftp_tree)
{
    proto_tree *root = dissect_a615a_header(tvb, pinfo, &offset, tftp_tree,
                                            "Load Configuration Initialization", "LCI");
    dissect_a615a_LUI_Common(tvb, pinfo, offset, root);
}
...
{
    proto_tree *root =
        dissect_a615a_header(tvb, pinfo, &offset, tftp_tree, "Load Downloading Media", "LND");
    dissect_a615a_LUI_Common(tvb, pinfo, offset, root);
}
...
{
    proto_tree *root =
        dissect_a615a_header(tvb, pinfo, &offset, tftp_tree, "Load Downloading Operator", "LNO");
    dissect_a615a_LUI_Common(tvb, pinfo, offset, root);
}
...
static void dissect_a615a_LUR(tvbuff_t *tvb, packet_info *pinfo, int offset, proto_tree *tftp_tree)
{
    gint end = -1;
    proto_tree *root =
        dissect_a615a_header(tvb, pinfo, &offset, tftp_tree, "Load Upload Request", "LUR");
...
static void dissect_a615a_LNL(tvbuff_t *tvb, packet_info *pinfo, int offset, proto_tree *tftp_tree)
{
    gint end = -1;
    proto_tree *root =
        dissect_a615a_header(tvb, pinfo, &offset, tftp_tree, "Load Downloading List", "LNL");
    ...
    for (unsigned i = 0; i < count; i++) {
        ...
        proto_tree_add_uint(part_root, hf_a615a_file_description_length, tvb, offset, 1,
                            descLength);
        offset += 1;
        ...
        if (descLength > 0) {
            ...
            proto_tree_add_string(part_root, hf_a615a_file_description, tvb, offset, descLength,
                                  desc);
            offset += descLength;
        }
    }
...
static void dissect_a615a_LNR(tvbuff_t *tvb, packet_info *pinfo, int offset, proto_tree *tftp_tree)
{
    gint end = -1;
    proto_tree *root =
        dissect_a615a_header(tvb, pinfo, &offset, tftp_tree, "Load Downloading Request", "LNR");
...
static void dissect_a615a_LNS(tvbuff_t *tvb, packet_info *pinfo, int offset, proto_tree *tftp_tree)
{
    gint end;
    proto_tree *root =
        dissect_a615a_header(tvb, pinfo, &offset, tftp_tree, "Load Downloading Status", "LNS");
...
static void dissect_a615a_LNA(tvbuff_t *tvb, packet_info *pinfo, int offset, proto_tree *tftp_tree)
{
    gint end = -1;
    proto_tree *root =
        dissect_a615a_header(tvb, pinfo, &offset, tftp_tree, "Load Downloading Answer", "LNR");
...
static void dissect_a615a_a665_msg(tvbuff_t *tvb, packet_info *pinfo, int offset,
                                   const char *a665Str, proto_tree *tftp_tree)
{
    (void)proto_tree_add_subtree_format(tftp_tree, tvb, offset, -1, ett_a665_protocol_root, NULL,
                                        "A665 %s File", a665Str);
}
                                        ...
                                        proto_tree *tftp_tree, int suffix)
{
    switch (suffix) {
        ...
        case LUP: {
            dissect_a615a_a665_msg(tvb, pinfo, offset, "Load Upload Part (LUP, Data File)",
                                   tftp_tree);
            break;
        }
        case LUH: {
...
{
    proto_a615a = proto_register_protocol("Arinc 615a Protocol", /* name       */
                                          "A615a-3",             /* short name */
                                          "a615a"                /* abbrev     */
    );
    static hf_register_info hf[] = {
        ...
        {&hf_a615a_info_op_status,
         {"Info Op Status Code", "a615a.info.status_code", FT_UINT16, BASE_DEC,
          VALS(a615a_op_status_codes), 0x0, "A615a Information Operation Status Code", HFILL}},
        ...
        {&hf_a615a_upload_op_status,
         {"Upload Op Status Code", "a615a.upload.status_code", FT_UINT16, BASE_DEC,
          VALS(a615a_op_status_codes), 0x0, "A615a Upload Operation Status Code", HFILL}},
        {&hf_a615a_download_op_status,
         {"Download Op Status Code", "a615a.download.status_code", FT_UINT16, BASE_DEC,
          VALS(a615a_op_status_codes), 0x0, "A615a Download Operation Status Code", HFILL}},

        /* load target */
        {&hf_a615a_part_load_op_status,
         {"Part Load Op Status Code", "a615a.upload.status_code", FT_UINT16, BASE_DEC,
          VALS(a615a_op_status_codes), 0x0, "A615a Part Load Op Status Code", HFILL}},
        {&hf_a615a_load_ratio,
    ...
    static gint *ett[] = {
        &ett_a615a,           &ett_a615a_opt_root,      &ett_a615a_opt,         &ett_a615a_fragment,
        &ett_a615a_fragments, &ett_a615a_protocol_root, &ett_a665_protocol_root};
...
void proto_reg_handoff_a615a(void)
{
    ...
    heur_dissector_add("tftp2", dissect_a615a_heur, "22 Arinc 615a Protocol", "22 A615a-3",
                       proto_a615a, HEURISTIC_ENABLE);
